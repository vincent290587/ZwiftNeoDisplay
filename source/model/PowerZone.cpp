...
PowerZone::PowerZone(uint16_t ftp) : BinnedData(), m_ftp(ftp) {
	m_tot_time = 0;
	m_last_bin = 0;
	m_last_timestamp = 0;

	m_last_bin_progress_pc = 0.f;
...
void PowerZone::addPowerData(uint16_t pw_meas, uint32_t timestamp) {
	...
	for (int i=0; i < PW_ZONES_NB; i++) {
		...
		if (pw_meas >= pw_lims[i] * m_ftp &&
				pw_meas < pw_lims[i+1] * m_ftp) {

			m_pw_bins[i] += time_integ;

			m_last_bin = i;

			m_last_bin_progress_pc = regFenLim((float)pw_meas, pw_lims[i] * m_ftp, pw_lims[i+1] * m_ftp, 0.f, 100.f);
...
float PowerZone::getCurPC(void) {

	return m_last_bin_progress_pc;
}
